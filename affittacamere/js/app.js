(function () {
  ...
  var FAQ_DEFS = [
    ...
    { q: { it: 'Serve un documento per prenotare?', en: 'Do I need an ID document to book?' },
      a: { it: 'Sì: per legge dobbiamo raccogliere i dati del documento d\'identità di tutti gli ospiti prima del check-in, tramite un link sicuro che ricevi dopo la richiesta di prenotazione.', en: 'Yes: by law we must collect ID document details for every guest before check-in, via a secure link you receive after your booking request.' } },
    ...
    privacy: {
      ...
      paragraphs: {
        it: [
          ...
          'Base giuridica: la raccolta dei dati anagrafici e del documento d\'identità NON si basa sul consenso ma sull\'adempimento di un obbligo di legge — le comunicazioni obbligatorie di pubblica sicurezza (Alloggiati Web, Questura), la comunicazione delle presenze e la riscossione della tassa di soggiorno (Comune di Monopoli), e le rilevazioni statistiche regionali (flussi ISTAT). I dati di contatto raccolti alla richiesta di prenotazione si basano invece sull\'esecuzione di un accordo precontrattuale con te.',
    ...
    termini: {
      ...
      paragraphs: {
        it: [
          ...
          'Ogni ospite che soggiorna deve fornire i propri dati anagrafici e una foto del documento d\'identità prima del check-in, tramite il link ricevuto dopo la richiesta di prenotazione: è un obbligo di legge, non facoltativo.',
        ...
        en: [
          ...
          'Every guest staying must provide their personal details and a photo of their ID document before check-in, via the link received after the booking request: this is a legal obligation, not optional.',
  ...
  function roomCardHtml(id, room) {
    ...
    return (
      '<div class="card" data-room-card data-room-id="' + id + '">' +
        '<div class="room-card-media">' +
          ...
          '<span class="room-card-tag" style="background:#EAF6FC; color:#2C8FC9;">€' + room.nightlyPrice + escapeHtml(t('room.per_night')) + '</span>' +
        '</div>' +
        '<div class="room-card-body">' +
          ...
          '<div class="room-card-badges">' +
            '<span class="room-card-type-badge">👤 ' + escapeHtml(guestsLabel) + '</span>' +
            (room.balcony && room.balcony !== 'nessuno' ? '<span class="room-card-balcony">' + escapeHtml(t('balcony.badge_' + room.balcony)) + '</span>' : '') +
          '</div>' +
          '<button type="button" class="btn room-card-cta" data-room-card-view>' + escapeHtml(t('room.view_singola')) + '</button>' +
        '</div>' +
      '</div>'
    );
  }
  ...
  function renderRooms() {
    ...
    container.innerHTML =
      ...
      '<div class="urgency-banner">' +
        ...
        '</div>' +
        '<a href="#stanze" class="btn btn-urgency">' + escapeHtml(t('urgency.rooms_cta')) + '</a>' +
      '</div>';
  }
  ...
  function guestsStepHtml() {
    ...
    return (
      '<div class="booking-summary">' + formatDateLabel(state.selectedCheckIn) + ' → ' + formatDateLabel(state.selectedCheckOut) + '</div>' +
